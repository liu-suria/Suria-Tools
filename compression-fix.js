(()=>{
  ...
  function renderCompress(root){
    root.innerHTML=`<div class="image-studio"><section class="studio-panel"><label class="drop-zone" for="compressFiles"><span class="drop-icon">＋</span><strong>选择或拖入图片</strong><small>支持 JPG、PNG、WebP，可多选</small><input id="compressFiles" type="file" accept="image/jpeg,image/png,image/webp" multiple></label><div class="control-grid"><label>输出格式<select id="compressType"><option value="auto">智能选择更小格式</option><option value="image/jpeg">JPG</option><option value="image/webp">WebP</option><option value="image/png">PNG</option></select></label><label>压缩质量 <output id="qualityText">78%</output><input id="compressQuality" type="range" min="20" max="95" value="78"></label><label>最大边长<select id="compressMax"><option value="0">保持原尺寸</option><option value="4096">4096 px</option><option value="2560">2560 px</option><option value="1920" selected>1920 px</option><option value="1280">1280 px</option><option value="800">800 px</option></select></label><label class="check-row"><input type="checkbox" checked disabled> 结果不得大于原图</label></div><div class="actions"><button id="startCompress" class="primary">开始压缩</button><button id="clearCompress" class="secondary">清空</button></div></section><section class="studio-panel"><div class="studio-heading"><div><h3>图片列表</h3><p id="compressSummary">上传后立即显示预览</p></div><button id="downloadAllCompress" class="secondary hidden">逐个下载全部</button></div><div id="compressResults" class="result-list empty-state">请选择图片</div></section></div>`;
    ...
    input.onchange=async()=>{cleanup();items=[...input.files].filter(valid).map(file=>({file,preview:URL.createObjectURL(file),done:false}));draw();for(const x of items){try{const img=await loadImage(x.file);x.width=img.naturalWidth;x.height=img.naturalHeight}catch{}draw()}summary.textContent=`已选择 ${items.length} 张，共 ${fmt(items.reduce((s,x)=>s+x.file.size,0))}`;all.classList.add('hidden')};
    ...
    root.querySelector('#startCompress').onclick=async()=>{
      ...
      for(const x of items){
        try{
          ...
          if(chosen==='auto'){if(x.file.type==='image/png')await add('image/webp');else{await add('image/webp');await add('image/jpeg')}}else await add(chosen);
      ...
      summary.textContent=`完成 ${items.filter(x=>x.done).length} 张 · ${fmt(before)} → ${fmt(after)} · ${saved}`;
      all.classList.toggle('hidden',!items.some(x=>x.done));
    };
